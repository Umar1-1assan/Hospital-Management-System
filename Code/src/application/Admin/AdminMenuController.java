...
public class AdminMenuController implements CotrollerIF
{
	private AdminManager adminManager;
	private User user;
	private ObservableList<Doctor> obsList = FXCollections.observableArrayList();
    ...
    private Button LogOut_btn;
	   ...
	   @FXML
	    ...
	    @FXML
	    private Button LabTech;

	    @FXML
	    private TableView<Doctor> List;
	    ...
	    @FXML
	    private TableColumn<Doctor,String> name_t;
	    ...
	    @FXML
	    private TableColumn<Doctor,String> username_t;
	    ...
	    {
	    	Doctor d = this.List.getSelectionModel().getSelectedItem();
	    	if(d==null)
	    	{
	    	...
	    	if(this.adminManager.removeDoctor(d.getUsername()))
	    	{
	    ...
	    @FXML
	    void Doctor(MouseEvent event) {
	    	this.bp.setCenter(ap);
	    }


	    @FXML
	    void Lab_Tech(MouseEvent event) throws ClassNotFoundException, SQLException {
	    	this.loadPage("Lab_Tech");
	    }

	    @FXML
	    void Nurse(MouseEvent event) throws ClassNotFoundException, SQLException {
	    	this.loadPage("Nurse");
	    }

	    @FXML
	    void Pharmacist(MouseEvent event) throws ClassNotFoundException, SQLException {
	    	this.loadPage("Pharmacist");
	    }

	    @FXML
	    void Receptionist(MouseEvent event) throws ClassNotFoundException, SQLException {
	    	this.loadPage("Receptionist");
	    }
	    ...
	    void add_update(MouseEvent event) throws ClassNotFoundException, SQLException 
	    {
	    	   try {
	    	        ...
	    	        boolean success = adminManager.addUpdateDoctor(
	    	            name_f.getText().trim(),
	    	            addr_f.getText().trim(),
	    	            cnic_f.getText().trim(),
    ...
    private void loadPage (String page) throws ClassNotFoundException, SQLException 
	{
        ...
        try {
            root = loader.load();
            CotrollerIF cotroller = loader.getController();
            cotroller.setUser(user);
            cotroller.setAdminManager(adminManager);
            cotroller.loadDeatails();
	...
	public void loadDeatails() throws ClassNotFoundException, SQLException 
	{
		...
		ArrayList<Doctor> doctors = this.adminManager.getDoctors();
        obsList.clear(); 
    ...
    void search(MouseEvent event) throws ClassNotFoundException, SQLException 
    {
    	...
    	ArrayList<Doctor> doctors = this.adminManager.getDoctors(searchname);
        obsList.clear(); 
        obsList.addAll(doctors); 
        List.setItems(obsList); 
    ...
    {
        Doctor selectedDoctor = List.getSelectionModel().getSelectedItem();
        ...
        try {
            Doctor doctor = this.adminManager.getDoctor(username);
            if (doctor != null) 
            {
                ...
                Speciality_f.setText(doctor.getSpeciality());
